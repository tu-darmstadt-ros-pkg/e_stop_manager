    ...
    : node_( std::make_shared<rclcpp::Node>( "e_stop_manager", options ) )
{
  ...
  if ( node_->get_parameter( "e_stop_topics", e_stop_topics_param ) &&
       e_stop_topics_param.get_type() == rclcpp::ParameterType::PARAMETER_STRING_ARRAY ) {
    e_stop_topics = e_stop_topics_param.as_string_array();
  } else {
    RCLCPP_ERROR( rclcpp::get_logger( "e_stop_manager" ),
                  "Parameter e_stop_topic not provided or is not a list! E-stops will only be "
                  "published on e-stop-list topic!" );
  }

  if ( node_->get_parameter( "e_stop_names", e_stop_topics_param ) &&
       e_stop_topics_param.get_type() == rclcpp::ParameterType::PARAMETER_STRING_ARRAY ) {
  ...
  for ( const auto &e_stop_name : e_stop_names ) {
    ...
    node_->declare_parameter<std::string>(
        e_stop_name + "_topic", e_stop_topics.empty() ? "missing_topics" : e_stop_topics[0] );
  }
  std::map<std::string, std::vector<std::string>> e_stops_per_topic;
  std::map<std::string, bool> e_stops_start_values;
  for ( const auto &e_stop_name : e_stop_names ) {
    e_stops_start_values[e_stop_name] =
        node_->get_parameter( e_stop_name + "_start_value" ).as_bool();
  ...
  for ( auto &entry : e_stop_topics ) {
    std::shared_ptr<rclcpp::Publisher<std_msgs::msg::Bool>> pub =
        node_->create_publisher<std_msgs::msg::Bool>( entry, 5 );
  ...
  for ( auto &pub : e_stop_pub_ ) {
    pub.second.erase(
        std::remove_if(
            pub.second.begin(), pub.second.end(),
            [&]( const std::string &name ) {
              auto it =
                  std::find( e_stop_list_msg_.names.begin(), e_stop_list_msg_.names.end(), name );
              if ( it == e_stop_list_msg_.names.end() ) {
                RCLCPP_WARN(
                    rclcpp::get_logger( "e_stop_manager" ),
                    "The e_stop named \"%s\" was in e_stop_topics but not in e_stop_list. Ignored.",
                    name.c_str() );
                return true;
              }
              return false;
    ...
    std::shared_ptr<e_stop_manager_msgs::srv::SetEStop::Response> response )
{
  ...
  RCLCPP_INFO( rclcpp::get_logger( "e_stop_manager" ), "EStop '%s' sent value: %d.",
  ...
  for ( size_t i = 0; i < e_stop_list_msg_.names.size(); ++i ) {
    if ( e_stop_list_msg_.names[i] == request->name ) {
      ...
      if ( e_stop_list_msg_.values[i] == request->value ) {
        RCLCPP_INFO( rclcpp::get_logger( "e_stop_manager" ),
                     "Do nothing. Requested state already true" );
  ...
  if ( !name_found ) {
    ...
    RCLCPP_ERROR( rclcpp::get_logger( "e_stop_manager" ), "EStop : '%s' not found!",
                  request->name.c_str() );
...
void EStopManager::publishEStops( bool force_e_stop )
{
  ...
  for ( auto &pub : e_stop_pub_ ) {
    ...
    msg->data =
        force_e_stop ||
        std::any_of( pub.second.begin(), pub.second.end(), [&]( const std::string &name ) {
